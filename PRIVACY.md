...
Only from the extension’s background worker:
...
- **Site currency hints:** `https://fun2code852.github.io/my-unit-config/site-currencies.json`, checked about once per hour. It maps public hostnames to the currency of an ambiguous `$` or `¥` symbol. The request does not include the page you are visiting.

No browsing history is included in those requests.

## What is stored on your machine

Your unit (custom name/price/currency or Yahoo ticker and last quote), FX cache, cached site-currency hints, default and per-site `$` / `¥` settings, and paused hostnames, in `chrome.storage.local`.
...
- **alarms** — refresh FX and site-currency hints periodically, and Yahoo (~15 min) only while a live ticker is saved.
