...
(function (root) {
  ...
  function resolveSymbolPrefix(prefix, ctx) {
    ...
    if (dollarFamily) {
      ...
      return UCE.resolveDollar(ctx.hostname, ctx.defaultDollar, ctx.overrides, ctx.pageDollar);
    }
    const yenFamily =
    ...
    if (yenFamily) {
      ...
      if (forced) return forced;
      if (prefix.currency) return prefix.currency;
      return UCE.resolveYen(ctx.hostname, ctx.defaultYen, ctx.yenOverrides, ctx.pageYen);
    }
    return prefix.currency || null;
  }
  ...
  function addRangeTail(out, source, start, currency) {
    const match = RANGE_TAIL_RE.exec(source.slice(start));
    if (!match) return;
    const numberStart = start + match[0].length - match[2].length;
    addMatch(out, numberStart, numberStart + match[2].length, match[2], UCE.parseNumber(match[2]), currency);
  }

  function addRangeHead(out, source, end, currency) {
    const match = RANGE_HEAD_RE.exec(source.slice(0, end));
    if (!match) return;
    const numberStart = end - match[0].length + match[0].indexOf(match[1]);
    addMatch(out, numberStart, numberStart + match[1].length, match[1], UCE.parseNumber(match[1]), currency);
  }

  /**
   * @param {string} text
   * @param {{ hostname?: string, defaultDollar?: string, overrides?: Record<string,string>, defaultYen?: string, yenOverrides?: Record<string,string>, pageDollar?: string, pageYen?: string }} ctx
   */
  UCE.findPrices = function findPrices(text, ctx) {
    ...
    while ((m = PREFIX_RE.exec(source))) {
      ...
      addRangeTail(found, source, m.index + m[0].length, currency);
    }
    ...
    while ((m = SUFFIX_RE.exec(source))) {
      ...
      addMatch(found, m.index, m.index + m[0].length, m[0], amount, currency);
      addRangeHead(found, source, m.index, currency);
    }
