(function (root) {
  ...
  UCE.formatCount = function formatCount(count) {
    ...
    if (count >= 1) {
      ...
      return groupThousands(trimFloat(count, decimals));
    }
    if (count >= 0.01) return groupThousands(trimFloat(count, 3));
  ...
  /** 1000 → 1,000; 1000.4 → 1,000.4. Decimal stays a period. */
  ...
  UCE.groupThousands = groupThousands;
  ...
  UCE.formatQuote = function formatQuote(price, priceHint) {
    ...
    return groupThousands(Number(price).toFixed(hint));
  };
